...
CREATE TABLE instruktorzy_stopnie (
	id_instruktora INT REFERENCES instruktorzy ON DELETE CASCADE NOT NULL ,
...
CREATE TABLE dzieci_grupy (
    id_klienta INT REFERENCES klienci ON DELETE CASCADE NOT NULL,
    id_grupy INT REFERENCES grupy ON DELETE CASCADE NOT NULL 
);
...
create table lista_oczekujących (
    id_klienta int references klienci(id_klienta) ON DELETE CASCADE NOT NULL ,
    id_odznaki int references odznaki(id_odznaki) NOT NULL,
    data_rozpoczecia DATE NOT NULL,
    primary key(id_klienta, id_odznaki, data_rozpoczecia)
);
